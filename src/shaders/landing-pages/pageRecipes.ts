...
/**
 * The engagement invitation. The page carries a second, Arabic face of its
 * own; that stack is pinned under `html[dir="rtl"]`, whose specificity beats
 * the `:root` written here, so retuning the Latin typography never reaches
 * the Arabic setting.
 */
export const ENGAGEMENT_TYPOGRAPHY: PageTypographyRecipe = {
  headingFonts: [
    { value: "instrument-serif", label: "Instrument Serif", stack: "\"Instrument Serif\", \"New York\", Georgia, serif", google: "Instrument+Serif" },
    { value: "newsreader", label: "Newsreader", stack: "\"Newsreader\", \"New York\", Georgia, serif", google: "Newsreader:wght@200..700" },
    { value: "cormorant", label: "Cormorant Garamond", stack: "\"Cormorant Garamond\", Georgia, serif", google: "Cormorant+Garamond:wght@300..700" }
  ],
  bodyFonts: [
    { value: "newsreader", label: "Newsreader", stack: "\"Newsreader\", \"New York\", Georgia, serif", google: "Newsreader:wght@200..700" },
    { value: "instrument-serif", label: "Instrument Serif", stack: "\"Instrument Serif\", \"New York\", Georgia, serif", google: "Instrument+Serif" },
    { value: "onest", label: "Onest", stack: "'Onest', system-ui, -apple-system, 'Helvetica Neue', sans-serif", google: "Onest:wght@100..900" }
  ],
  ...
  bodyWeight: "400",
  primaryColor: "#2b2721",
  headingSize: [20, 30, 46],
  bodySize: [14, 20, 26],
  headingLetterSpacing: [-0.04, 0.01, 0.12],
  css: (type) => `
:root {
  --ink: ${type.primary};
  ...
  --earth: ${type.retone("#9a6a3e")};
  --display: ${type.heading};
  --font: ${type.body};
}
body { font-weight: ${type.bodyWeight}; }
.top .name, .plate .t, .row-v { font-family: ${type.heading}; font-weight: ${type.headingWeight}; }
.top .name { font-size: clamp(20px, 2.4vw, ${px(type.headingSize)}); letter-spacing: ${n(type.headingLetterSpacing)}em; }
.plate .t { font-size: clamp(19px, 2.1vw, ${px(type.headingSize * 26 / 30)}); }
.bio { font-size: clamp(${px(Math.max(15, type.bodySize - 3))}, 1.7vw, ${px(type.bodySize)}); }
.row-v { font-size: clamp(17px, 1.8vw, ${px(type.bodySize + 2)}); }
`
};
